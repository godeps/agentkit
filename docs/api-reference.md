...
- `type Request` (`options.go:258`) includes `Prompt`, `ContentBlocks []model.ContentBlock`, `Mode`, `SessionID`, `RequestID string`, `Model ModelTier`, `EnablePromptCache *bool`, `OutputSchema *model.ResponseFormat`, `Traits`, `Tags`, `Channels`, `Metadata`, `TargetSubagent`, `ToolWhitelist`, `ForceSkills`. `request.normalized` fills `SessionID`, merges `Mode`, trims prompt, auto-generates `RequestID` if empty, and deep-clones `OutputSchema`.
