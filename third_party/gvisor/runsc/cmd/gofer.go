...
func (g *Gofer) setupRootFS(spec *specs.Spec, conf *config.Config, goferToHostRPC *urpc.Client) error {
	...
	if !conf.TestOnlyAllowRunAsCurrentUserWithoutChroot {
		...
		if err := specutils.SafeSetupAndMount(specutils.ExePath, "/proc/fs/runsc", "bind", unix.MS_BIND|unix.MS_RDONLY, procPath); err != nil {
			return fmt.Errorf("mounting runsc in gofer root: %v", err)
		}
		root = "/proc/fs/root"
