...
func setUpChroot(spec *specs.Spec, conf *config.Config) error {
	...
	if err := specutils.SafeSetupAndMount(specutils.ExePath, filepath.Join(chroot, "runsc"), "bind", unix.MS_BIND|unix.MS_RDONLY, "/proc"); err != nil {
	...
	if err := pivotRoot(chroot); err != nil {
		return err
	}
